...
const request = async <T = any>(
  ...
  try {
    ...
    try {
      ...
      if (parseError instanceof Error && (parseError as ApiError).status) {
        throw parseError;
      }
      const error = new Error("Failed to parse response") as ApiError<T>;
      error.status = response.status;
      error.data = parseError as T;
      throw error;
    }
  } catch (fetchError) {
    console.error("[v0] API request failed:", fetchError);
    if (fetchError instanceof Error && (fetchError as ApiError).status) {
      throw fetchError;
    }
    const error = new Error(
